...
|      항목       | ZFS                                         | LVM                      | NFS                                         | Ceph                                         |
|:---------------:|:--------------------------------------------:|:------------------------:|:-------------------------------------------:|:--------------------------------------------:|
| 종류            | 파일 시스템 + 볼륨 매니저                    | 볼륨 매니저              | 파일 공유 프로토콜                          | 분산 스토리지 시스템                         |
| 주 용도         | 디스크 관리 및 데이터 무결성 보장            | 물리 디스크를 논리적으로 관리 | 네트워크를 통한 파일 공유                   | 대규모 데이터 저장소 구축                    |
| 설치 위치       | 로컬 서버 (커널 모듈 또는 패키지 설치)       | 로컬 서버 (커널 기본 지원)   | 서버-클라이언트 환경 (서버/클라이언트 모두 설정) | 여러 서버(노드) 설치 및 연결               |
| 데이터 무결성   | 자체 체크섬으로 자동 검증·복구               | 없음 (파일 시스템에 의존) | 기본 없음 (네트워크 전송 안정성에 의존)     | 자동 복제와 자가 치유(Self-healing) 기능     |
| 스냅샷 기능     | 지원 (빠르고 효율적)                         | 지원 (별도로 설정)       | 없음 (백업으로 대체)                        | 지원 (RADOS 기반 스냅샷)                     |
| 확장성          | 풀(Pool) 단위로 확장 가능                    | 볼륨 그룹에 디스크 추가     | 서버 증설로 간접 확장 가능                  | 노드 추가만으로 수평 확장 가능 (scale-out)   |
| 백업/복구       | 스냅샷과 복제로 매우 강력                    | 스냅샷은 있으나 관리 복잡 | 수동 백업 필요                              | 자동 복제 및 리밸런싱                        |
| 성능 특성       | RAM 사용량 많음 (ARC 캐시) / 읽기·쓰기 최적화 | 파일 시스템에 따라 다름    | 네트워크 속도에 따라 다름                   | 수백 PB까지 확장 가능, 네트워크 품질에 의존 |
| 운영체제 호환성 | Linux, 일부 macOS/Windows 등| Linux (LVM2)             | 대부분의 OS                                 | Linux 계열(주로 Ubuntu, CentOS, RHEL)        |
| 관리 복잡성     | 중간 (초기 셋업 후 안정적)                   | 간단~중간 (명령어 숙련 필요) | 간단 (export, mount 설정)                  | 매우 복잡 (운영 및 유지보수 전문 지식 필요) |
| 적합한 환경     | 고성능 서버, NAS, 대용량 스토리지            | 유연한 디스크 관리가 필요한 서버 | 파일 공유가 필요한 작은 조직~기업         | 대규모 클라우드, 빅데이터, 고가용성 시스템  |
